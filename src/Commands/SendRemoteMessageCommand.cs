        ...
        protected override void ProcessRecord()
        {
            ...
            if (string.IsNullOrEmpty(ComputerName))
            {
                if (SessionId is null)
                {
                    ...
                    {
                        MessageResponse[] result = unWrapper.SendRemoteMessage(IntPtr.Zero, null, Title, Message, nativeStyle, Timeout, Wait);
                        result.Where(q => q is not null).ToList().ForEach(x => WriteObject(x));
                    }
                }
                else
                {
                    MessageResponse[] result = unWrapper.SendRemoteMessage(IntPtr.Zero, SessionId, Title, Message, nativeStyle, Timeout, Wait);
                    result.Where(q => q is not null).ToList().ForEach(x => WriteObject(x));
                }
            }
            else
            {
                ...
                if (SessionId is null)
                {
                    ...
                    {
                        MessageResponse[] result = unWrapper.SendRemoteMessage(session.SessionHandle.DangerousGetHandle(), null, Title, Message, nativeStyle, Timeout, Wait);
                        result.Where(q => q is not null).ToList().ForEach(x => WriteObject(x));
                    }
                }
                else
                {
                    ...
                    {
                        MessageResponse[] result = unWrapper.SendRemoteMessage(session.SessionHandle.DangerousGetHandle(), SessionId, Title, Message, nativeStyle, Timeout, Wait);
                        result.Where(q => q is not null).ToList().ForEach(x => WriteObject(x));
                    }
                }
            }
