...
Describe "Get-ServiceSecurity by name" {
...
Describe "Get-ServiceSecurity by service controller" {
    It "Retrieves the 'ServiceSecurity' object for a service" {
        $service = Get-Service -Name 'WindowsUtils_Pester_Service'
        
        $service_security = Get-ServiceSecurity -InputObject $service
        $service_security | Should -BeOfType ([WindowsUtils.AccessControl.ServiceSecurity])
        
        $service_security = $service | Get-ServiceSecurity
        $service_security | Should -BeOfType ([WindowsUtils.AccessControl.ServiceSecurity])
    }
    It "Always returns a valid 'SDDL', 'Access', 'AccessToString, 'Name' and 'Owner'." {
        $service = Get-Service -Name 'WindowsUtils_Pester_Service'
        ...
        foreach ($dummy in 1..2) {
            if ($dummy -eq 1) { $service_security = Get-ServiceSecurity -InputObject $service }
            else { $service_security = $service | Get-ServiceSecurity }
            # Testing Name.
    ...
    It "Returns a valid 'SystemAuditRule' array when called with '-Audit'." {
        ...
        $service_security = Get-ServiceSecurity -InputObject $service -Audit
        $service_security.Audit | Should -BeOfType ([WindowsUtils.AccessControl.ServiceAuditRule])
        $service_security.Audit.Count | Should -EQ 1
        $service_security.Audit[0] | Should -EQ ([WindowsUtils.AccessControl.ServiceAuditRule]::new('Everyone', 'AllAccess', 'Failure'))

        $service_security = $service | Get-ServiceSecurity -Audit
        $service_security.Audit | Should -BeOfType ([WindowsUtils.AccessControl.ServiceAuditRule])
        $service_security.Audit.Count | Should -EQ 1
        $service_security.Audit[0] | Should -EQ ([WindowsUtils.AccessControl.ServiceAuditRule]::new('Everyone', 'AllAccess', 'Failure'))
    }
